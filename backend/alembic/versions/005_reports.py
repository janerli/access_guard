...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("data_source", sa.Enum("postgres", "elasticsearch", "combined",
                  name="report_data_source", create_type=False), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column("format", sa.Enum("pdf", "xlsx", "csv", name="report_format", create_type=False), nullable=False),
        sa.Column("status", sa.Enum("pending", "generating", "ready", "failed",
                  name="report_status", create_type=False), nullable=False, server_default=sa.text("'pending'")),
    ...
    op.create_table(
        ...
        sa.Column("template_id", postgresql.UUID(as_uuid=True),
                  sa.ForeignKey("report_templates.id"), nullable=False),
        sa.Column("parameters", postgresql.JSONB(), nullable=False, server_default=sa.text("'{}'")),
        sa.Column("format", sa.Enum("pdf", "xlsx", "csv", name="report_format", create_type=False),
                  nullable=False, server_default=sa.text("'xlsx'")),
